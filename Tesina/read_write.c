...
loff_t vfs_setpos(struct file *file, loff_t offset, loff_t maxsize)
{
	...
	if (offset != file->f_pos) {
		...
		{
			down_write(file->sess_so->sem);
			file->sess_so->offset = file->f_pos;
			printk("[VFS_SET_POS]Just fixed our file position in the session to %lld!\n", file->f_pos);
			up_write(file->sess_so->sem);
		}
		file->f_version = 0;
	}
	return offset;
...
static inline loff_t file_pos_read(struct file *file)
{
	...
	if(file->f_flags & O_SESSION)
	{
		printk(KERN_DEBUG "[FILE_POS_READ]Reading pos from session %llu\n", file->sess_so->offset);
		return file->sess_so->offset;
	}
	return file->f_pos;
}
...
static inline void file_pos_write(struct file *file, loff_t pos)
{
	...
	if(file->f_flags & O_SESSION)
	{
		printk("[FILE_POS_WRITE]Fixing the offset to %lld\n", pos);
		file->sess_so->offset = pos;
	}
}
